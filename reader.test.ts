...
Deno.test("get latest post without a date", async () => {
  ...
  if (text !== 'with a timestamp') throw Error(`post body should be "with a timestamp", not ${JSON.stringify(text)}`)
  post.body.close()
})
...
Deno.test("get latest post before a time", async () => {
  ...
  const bytes = await Deno.readAll(post.body)
  const text = new TextDecoder("utf-8").decode(bytes)
  if (text !== 'hi there!') throw Error(`post body should be "hi there!", not ${JSON.stringify(text)}`)
  post.body.close()
})
